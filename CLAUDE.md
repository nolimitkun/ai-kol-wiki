...
1. `uv run scripts/fetch.py <url> --kol <slug>` — 抓元数据和字幕，生成 `sources/.../transcript.md`。
   ...
   - 若完全无字幕：用 `--transcribe` 一键下载音频 + faster-whisper 本地转录。模型体积大不进内联依赖，按需注入：`uv run --with faster-whisper scripts/fetch.py <url> --kol <slug> --transcribe`（首次会拉模型约 1.5GB；faster-whisper 基于 CTranslate2，不依赖 torch，比原版快约 4 倍，默认模型 large-v3-turbo）。
     ...
     - **访谈类建议再加 `--diarize` 做说话人分离**（多注入 `--with pyannote.audio`）：在转录稿里插入 `SPEAKER_00:` / `SPEAKER_01:` 标签，让"谁说的"有机械依据，而不是靠读内容猜。首次需用同一 HF 账号接受 `pyannote/speaker-diarization-community-1` 的条款（默认模型；条款按账号生效，换 token 不带过去），再 `export HF_TOKEN=<token>`；无 GPU 或未配 token 时自动跳过，仍产出无标签转录稿。
...
只**新增** `speakers.md`（pyannote 的说话占比 + 轮次表），`transcript.md` 逐字节不动
（脚本用 sha256 前后比对做硬保证）。

**脚本本身不跑 whisper**，只做分离：要把 `SPEAKER_XX` 标签嵌回转录稿正文才需要
重跑 whisper 取片段级时间轴，而重跑会让切分漂移，使 wiki 里已有的 `[HH:MM:SS]`
引用失准。所以这里只出 sidecar——只增不改，旧引用全部继续有效。
sidecar 怎么和转录稿对照着读，见上面 Ingest 第 2 步。
...
- 确认 index.md 覆盖所有页面
- 确认引用时间戳都是信源转录稿里逐字存在的锚点（`lint.py` 自动查，规则见下方「写作约定」）

## 写作约定

- **观点必须可溯源**：wiki 中每条实质性观点后面标 `（[视频页](../videos/xxx.md) HH:MM:SS）`。
- **信源要写成链接，不能只写在标题文字里**：一个小节的引用若只写时间戳、不逐条带视频链接，就在小节标题下紧跟一行 `来源：[频道 / 主体](../videos/xxx.md)`（多个视频用顿号并列）。
  - 「来源：」行管到下一条「来源：」行为止，`###` 子小节不声明就沿用上层；所以**换了视频的小节必须重新声明**，否则会继承上一节的信源。
  - 标题里的"（Decagon，2026-07）"、括号里的 `[No Priors 00:06:12]` 都不算声明——读者点不过去，巡检也认不出。曾有 93 条真锚点因此被判成"对不上信源"。
  - 同一行引用别的视频时（交叉引用、对照），把链接放进时间戳所在的括号里：`（[视频页](../videos/yyy.md) HH:MM:SS）`；链接在括号外的，时间戳会被算到本节声明的信源上。
  - 一个小节混用多个视频、且逐条都能带行内链接时，可以不写「来源：」行。
  - 整页只链接一个视频的页面不必写（默认就是它）；视频页默认是自己的转录稿。
  - `uv run scripts/lint.py` 按这套规则逐处核验：时间戳不是信源转录稿里的锚点报 🟠，找不到信源报 🟡。
- **时间戳必须照抄转录稿锚点**，写成 `（[HH:MM:SS]）` 或区间 `（[HH:MM:SS]–[HH:MM:SS]）`：
  - 只能用 `transcript.md` 里**真实存在**的 `[HH:MM:SS]` 行首锚点，逐字复制。不要自己心算、取整或插值——写 `（00:05）` 这类值在转录稿里搜不到，读者无法定位。
  - **一律写满 `HH:MM:SS`**，不写 `HH:MM`，更不写 `MM:SS`。这两种缩写彼此无法区分：`05:00` 既可读成 5 分 0 秒也可读成 5 小时，曾导致一整页 42 处引用被误算、全部指向同一时间点。
  - ⚠️ **锚点不代表秒级精度**：锚点间隔 60–68 秒，一个锚点标注的是**一整块约一分钟的文本**。写满秒数是为了能 grep 定位，不是说这条观点精确出现在那一秒。
  - 若某处内容超出转录稿结尾（转录稿比视频短），**保留粗略写法并注明**，不要硬套到最近的锚点上——宁可粗，也不要假精确。
