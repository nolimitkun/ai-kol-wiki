...
- **工程**：omni connector 统一传输（单卡 shared memory、多卡走 Mooncake）、控制面/数据面解耦、chunkwise 异步流式把**首包延迟从近 10 秒降到 <1 秒**、RTF<1（"响应快的 AI 能给情绪价值"）（00:28:38–00:33:47）。
- **DiT 加速**：TeaCache（layer 级跳步）+ block 级缓存；并行 USP+Ring Attention（正交）、HSDP、VAE patching、CFG parallel（最稳）；实测现代显卡算力过剩、diffusion 可 continuous batching；边缘设备做 module/layer-wise CPU offload（00:41:08–00:57:38）。
- **国产硬件插件**：CUDA/ROCm + 华为昇腾 MPU、百度昆仑，新硬件接入不改核心逻辑（00:24:27–00:25:30）。
- **未来**：streaming input 支持 world model / VLA、与 veRL 集成做多模态 RL、更多 multi-stage 模型（Bagel、Qwen-Omni、混元 80B）（01:03:51–01:08:55）。
